...
def load(opts, ptr, t):
  ...
  match despecialize(t):
    ...
    case Char()         : return int_to_char(opts, load_int(opts, ptr, 4))
...
def load_variant(opts, ptr, cases):
  ...
  return { case.label: load(opts, ptr, case.t) }
...
def match_case(v, cases):
  assert(len(v.keys()) == 1)
  key = list(v.keys())[0]
  matches = [i for i,c in enumerate(cases) if c.label == key]
  assert(len(matches) == 1)
  case_index = matches[0]
  return (case_index, v[key])
...
def lift(opts, vi, t):
  match despecialize(t):
    ...
    case Char()         : return int_to_char(opts, vi.next('i32'))
  ...
  class CoerceValueIter:
    def next(self, want):
      ...
      match (have, want):
        ...
        case ('i64', 'f32') : return reinterpret_i32_as_float(x)
